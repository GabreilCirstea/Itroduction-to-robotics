...
//function and variables from other files
...
const int animColors[animColorsNo][COLOR_CODE]={
  {255,0,0},  //red
  {0,255,0},  //green
  {0,0,255},  //blue
  {255, 128, 0},
...
void setAnimColor(const int colors[COLOR_CODE]){
...
void playStrobe(AnimInfo& animStep){
  //the color
  if(millis() - animStep.lastTimeAnim > animStep.timeInterv){
    if(brightness < 5 || brightness > MAX_BRIGHT)
      animStep.colorStep *= -1;
      
    brightness += animStep.colorStep;
...
void initStrobeAnim(){
  ...
  animStep.lastTimeAnim = 0;
  animStep.timeInterv = 500;   //time to modify the color
  animStep.animIndex = 0;
  animStep.colorStep = 5;     //not needed any more
}
...
void playFade(AnimInfo& animStep){
  ...
  if(millis() - animStep.lastTimeAnim > 100){ //a shor interv to anim
    bool change = false;
...
void playSmooth(AnimInfo& animStep){
  ...
  //TO DO:sa facem culori plimbarete
}
